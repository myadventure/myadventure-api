...
from app.mod_tracker.models import Point, Config, User

mod_tracker = Blueprint('tracker', __name__, url_prefix='')

@mod_tracker.route('/api/v1/point/route/load', methods=['POST'])
def load_route():
    try:
...
@mod_tracker.route('/api/v1/point/<type>', methods=['GET'])
def list_point(type):
    points_dict = None
...
@mod_tracker.route('/api/v1/point/<type>/<id>', methods=['GET'])
...
@mod_tracker.route('/api/v1/point/<type>/<id>', methods=['PUT'])
...
@mod_tracker.route('/api/v1/point/<type>', methods=['POST'])
def add_point(type):
    try:
        data = json.loads(request.data)
...
@mod_tracker.route('/api/v1/point/<type>/<id>', methods=['DELETE'])
...
@mod_tracker.route('/api/v1/config/<name>', methods=['GET'])
def get_config(name):
...
@mod_tracker.route('/api/v1/config', methods=['POST'])
def save_config():
    try:
        data = json.loads(request.data)
        name = data['name']
...
@mod_tracker.route('/api/v1/point/tracker/load', methods=['GET'])
def load_tracker():
...
@mod_tracker.route('/api/v1/user', methods=['POST'])
def add_user():
    try:
        data = json.loads(request.data)
        email = data['email']
...
@mod_tracker.route('/api/v1/point/flickr/load', methods=['GET'])
def load_flickr():
...
@mod_tracker.route('/api/v1/point/instagram/load', methods=['GET'])
def load_instagram():
